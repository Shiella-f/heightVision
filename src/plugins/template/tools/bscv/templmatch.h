...
namespace TIGER_BSVISION
{
    ...
    public:
        ...
        /*
            ...
            num_features: 最小模板点数。
            threshold_value:内外轮廓二值化阈值。
            threshold_type:0表示不区分，1表示大于阈值区域为有效区域，2表示小于阈值区域为有效区域。
        */
        virtual bool create_shape_model(cv::InputArray _image, cv::InputArray _mask = cv::Mat(), 
                                        float _angle_start = 0.0f, float _angle_extent = 360.0f, 
                                        float _angle_step = 1.0f, float _scale_min = 0.9, float _scale_max = 1.1, float _scale_step = 0.01f,
                                        float _weak_thresh = 30.0f, float _strong_thresh = 60.0f, 
                                        size_t _num_features = 64, 
                                        CPolarityType _type = cptIgnore) = 0;
        virtual bool create_one_shape_model(cv::InputArray _image, cv::InputArray _mask = cv::Mat(), 
                                        float _weak_thresh = 30.0f, float _strong_thresh = 60.0f, 
                                        size_t _num_features = 30, CPolarityType _type = cptIgnore) = 0;
        ...
        */
        ...
        /*
            ...
            num_features: 最小模板点数。
        */
        virtual std::vector<Match> find_pic_model(cv::InputArray _image, cv::InputArray _model, cv::InputArray _mask = cv::Mat(),
                                                  cv::InputArray _model_mask = cv::Mat(), float _threshold = 90.0f, 
                                                  float _angle_start = 0.0f, float _angle_extent = 360.0f, 
                                                  float _angle_step = 1.0f, float _scale_min = 0.9, 
                                                  float _scale_max = 1.1, float _scale_step = 0.01f,
                                                  float _weak_thresh = 30.0f, size_t _num_features = 64) = 0;
