...
// 实现工厂函数
IImageProcess* CreateImageProcessInstance()
{
...
cv::Mat ImageProcess::CalibImage(cv::Mat &originalImage)
{
    if(!originalImage.empty())
    {
        ...
        {
            //qInfo().noquote() << "相机标定数据未加载，返回原图";
            // cv::cvtColor(displayedFrame, displayedFrame, cv::COLOR_BGR2RGB);
            return displayedFrame;
        }
    } else 
    {
        qInfo().noquote() << "image is empty";
        return cv::Mat();
    }
}

void ImageProcess::loadCalibrationData()
{
    ...
    try {
        // 读取 9x9 振镜单应矩阵
        cv::FileStorage fs(calibMatrixPath.toStdString(), cv::FileStorage::READ);
        if (!fs.isOpened()) {
        ...
        if (HomographyMatrix.empty()) {
            ...
            m_is9_9CalibLoaded = false;
        } else {
            m_is9_9CalibLoaded = true;
